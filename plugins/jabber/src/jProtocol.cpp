...
void jProtocol::handlePresence (const Presence &presence)
{
	...
	if(jid == m_account_name)
	{
		if (presence.presence() != Presence::Unavailable)
		{
			...
			m_jabber_roster->addMyConnect(resource, presence.priority(), presence.presence());
			m_jabber_roster->changeItemStatus(jid+"/"+resource, presence.presence());
			if(!exist)
			{
	...
	else
	{
		...
		{
			m_jabber_roster->addResource(jid, resource, presence.priority(), presence.presence());
			const Capabilities *caps = presence.capabilities();
			jBuddy::ResourceInfo *info = m_jabber_roster->getBuddy(jid)->getResourceInfo(resource);
			if(caps)
			{
			...
			if (m_jabber_roster->getMaxPriorityResource(jid) == resource)
				m_jabber_roster->changeItemStatus(jid, presence.presence());
...
{
    Presence pres(jClient->presence().presence(), JID(toStd(jid)));
    jClient->send(pres);
}

void jProtocol::LogOutTransport(const QString &jid)
{
    Presence pres(Presence::Unavailable, JID(toStd(jid)));
    jClient->send(pres);
}
...
{
	QSettings *recent_settings = new QSettings(QSettings::defaultFormat(), QSettings::UserScope, "qutim/qutim."+m_profile_name+"/jabber."+m_account_name, "recent");
