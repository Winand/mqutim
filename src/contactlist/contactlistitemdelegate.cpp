...
/*	qutIM *main = AbstractLayer::instance().getParent();
	...
	qWarning() << main->paintEngine() << main_painter;*/
	...
	case 1:{
		...
		case 0:{
			QString type = index.data(Qt::UserRole+3).toString();
			style = m_styles[selection+type];
			break;
		}
		case -1:
			style = m_styles[selection+"separator"];
			break;
		case 1:
			style = m_styles[selection+"group"];
                        break;
		case 2:
			style = m_styles[selection+"account"];
			break;
		}
...
QPixmap ContactListItemDelegate::getAlphaMask(QPainter *painter, QRect rect, int type) const
{
	...
	if(type!=0)
                alpha_painter.drawRoundRect(QRect(QPoint(0,0),QSize(x2-1,y2-1)),10,50);
	switch(type)
	{
...
bool ContactListItemDelegate::setThemeStyle(QString path)
{
	...
	if(path.endsWith(".ListTheme"))
	{
		...
		m_style_colors["background"] = qvariant_cast<QColor>(style_hash["Background Color"]);
