...
jJoinChat::jJoinChat(jAccount *jabber_account, const QString &room, const QString &password, bool showBookmark, QWidget *parent) : QWidget(parent)
{
	...
	QSettings *recent_settings = new QSettings(QSettings::defaultFormat(), QSettings::UserScope, m_settings, "recent");
	...
	m_account_settings = new QSettings(QSettings::defaultFormat(), QSettings::UserScope, m_settings, "accountsettings");
...
void jJoinChat::on_saveButton_clicked()
{
	...
	{
		m_bookmark_storage->storeBookmarks(m_b_list.toStdList(), m_c_list.toStdList());
		m_jabber_account->getProtocol()->setRecentBookmarks(m_b_list.toStdList(), m_c_list.toStdList());
	}
